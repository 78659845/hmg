      ...
      <c:forEach items="${ model.data }" var="o" >
          ...
          <td>
          	<div class="button-group">
				...
				<a class="button border-main" style="display:none" href="javascript:void(0)" onclick="baocun(this)">
...
//$("#button border-main").click(function(){
//	var tr = $(this).parent().parent().parent();
//	var td3 = tr:eq(2);
//	var td4 = tr:eq(3);
//	var td5 = tr:eq(4);
//	var td6 = tr:eq(5);
	//$(this).html("<span class="icon-save"></span> 保存");
//});
		...
		$.ajax({
			...
			success : function(r){
				...
				for (var i = 0; i < list.length; i++){
					//console.log("id=" + list[i].gtid + "  name=" + list[i].name + "  desc=" + list[i].description);
				...
				$select.change(function(){
					...
					//选到select中的一项option
					var gtid = $(this).val();
					var t = $(this).find("option[value='" +gtid+ "']").text();
					//alert("你选中的是：" + t + "   id=" + gtid);
					updateGtid(gtid, $select);
				})
			}
		});
...
function updateGtid(gtid, $select){
	...
	alert("要修改的商品id:" + goodsid + "    商品的类别：" + gtid);
	//第1步：写一个web服务方法
	//第2步： 在这里写ajax请求，调用上面的方法
	//第3步：在业务层接口扩展一个更新商品类别的方法
	//第4步：在业务层实现类实现相应的方法
	//第5步：在数据层DAO接口扩展一个教师新村商品类别的方法
	//第6步：在DAO对旧的mapper文件中定义相应的update语句
	//第7步：在客户处端理服务端返回的结果 ，显示消息气泡
	
	$.ajax({
		url : "ws/hongyan/goods/update/typemap",
		type: "post",
...
function xiugai(a){
	...
	$(a).hide();
	//$(a).next("a").css("display","block");
	$(a).next("a").show();
}
function editTd($td){
	var $input = $("<input />");//jq文本框
	$input.val($td.text());//向文本框中加入td的内容
	$input.css("width", "80px").css("height", "25px");
	$td.html($input);//input标签替换td标签
}
function resetTd($input){
	var $td = $("<td><td/>");
	$td.text($input.val());
	$input.html($td);
}
...
function doPush(goodsid, a){
	...
	$.ajax({
		...
		url	: "admin/goods/push.php",//Servlet
		type: "POST",
